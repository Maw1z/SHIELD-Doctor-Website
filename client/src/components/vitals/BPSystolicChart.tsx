...
const chartConfig = {
  bp_systolic: { label: "Systolic", color: "hsl(24, 100%, 50%)" },
} satisfies ChartConfig;

export function BPSystolicChart() {
  ...
  const { chartData, average } = useMemo(() => {
    ...
    const avg = filtered.length
      ? filtered.reduce((s, d) => s + d.bp_systolic, 0) / filtered.length
      : 0;
        ...
        <div className="flex gap-1 mb-4 overflow-x-auto pb-1 no-scrollbar">
          {(
            ...
            <button
              ...
              className={`px-2 py-1 text-[9px] font-bold rounded shrink-0 transition-all ${timeRange === r ? "bg-orange-600 text-white shadow-sm" : "bg-slate-100 text-slate-500 hover:bg-slate-200"}`}
            >
              {r}
            </button>
          ))}
